...
if (isset($notelp)) {
    ...
    if (mysqli_num_rows($users) > 0) {
        while ($data = mysqli_fetch_array($users)) {
            ...
            $dendalain = mysqli_query($con, "SELECT SUM(nominal) as totaldenda FROM datadenda_lainlain WHERE id_users = $id AND status = 0");
            if (mysqli_num_rows($dendalain) > 0) {
                while ($rows = $dendalain->fetch_assoc()) {
                    $denda_lainlain = $rows['totaldenda'];
                }
            } else {
                $denda_lainlain = 0;
            }
            ...
            $gaji_pokok = $data['gaji_pokok'];
            $totalgaji = intval($gaji_pokok) - intval($denda) - intval($denda_lainlain) - $totalbpjs;
        }
        $hasil .= '<div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="form-label" style="font-size: 14px;">Total Denda Lain - Lain</label>
                                        <div name="total_dendalain" id="total_dendalain" class="form-control total_dendalain" style="font-size: 12px;" readonly>' . rupiah($denda_lainlain) . '</div>    
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group"> 
                                        <label class="form-label" style="font-size: 14px;">Total Denda Terlambat</label>
